...
<p align="center">
  ...
  <img alt="Version" src="https://img.shields.io/badge/Version-0.2.0-FC3C44">
</p>
...
- 顺序 / 列表循环 / 单曲循环；随机播放为抽袋（同一轮不重复），上一首按播放历史回退
- 下一首播放、播放队列
...
在线音质由 **对应平台账号权益** 决定，本软件只请求接口并按失败档位回退。
...
QQ 音乐（切到 QQ 音乐后，设置 → 音质偏好；与网易云档位分开保存）：

| 档位 | 说明 | 大致要求 |
|------|------|----------|
| `hires` | Hi-Res FLAC | 豪华 / SVIP |
| `lossless` | 无损 FLAC | VIP |
| `exhigh` | 高品质 320 kbps MP3 | 登录即可 |
| `standard` | 标准 128 kbps MP3 | — |
| `aac` | 流畅 96 kbps AAC | — |

无版权或需会员时会提示「需要会员或无版权」，并按失败档位向下回退。

每日推荐：登录后请求个性化接口（网易云日推、QQ 雷达 / 猜你喜欢）。未登录或接口失败时改用新歌或热歌，并在歌单标题标明，不会假装成日推。无账号时音质按档位回退是预期行为。
